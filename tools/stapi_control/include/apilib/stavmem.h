...
/* Parameters returned when getting memory block info */
typedef struct STAVMEM_BlockInfo_s
{
    STAVMEM_PartitionHandle_t PartitionHandle;  /* Partition in which the block was allocated*/
    STAVMEM_BlockHandle_t BlockHandle;          /* Block Handle */
    ST_DeviceName_t AllocatorName;              /* Allocator Name */
    STAVMEM_AllocMode_t AllocMode;              /* Way the block was allocated */
    U32   StartAddress;                         /* First physical address of the block */
    U32   Size;                                 /* Size of the block in bytes. 0 means full size */
    U32   UserSize;                             /* User specified size when alloc. 0 means full range */
    U32   Alignment;                            /* Alignement of the first address of the block */
    U32   AlignedStartAddress;                  /* First address of the block for the user (aligned) */
} STAVMEM_BlockInfo_t;
