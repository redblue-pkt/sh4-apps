...
static int usage(Context_t *context, char *prg_name, char *cmd_name)
{
	...
	for (i = 0; ; i++)
	{
		if (vK912Args[i].arg == NULL)
		{
			break;
		}
		if ((cmd_name == NULL) || (strcmp(cmd_name, vK912Args[i].arg) == 0) || (strstr(vK912Args[i].arg_long, cmd_name) != NULL))
		{
			printf("%s   %s   %s\n", vK912Args[i].arg, vK912Args[i].arg_long, vK912Args[i].arg_description);
		}
	}
...
{  // -v command: OK
	...
	{
		fprintf(stderr, "Error reading version from front processor\n");
		*version = 0;
	}
	return 0;
}
